...
for ep in range(c_no_of_eps):
    ...
    observation, _ = env.reset()  # Với Gymnasium (Gym mới)
    current_state = convert_state(observation)
    ep_reward = 0
    ...
    while not done:
        ...
        # Hành động theo action đã lấy (sử dụng từ thư viện gym)
